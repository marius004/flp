...
ackermannf = fix (\f m n -> if m == 0 then n + 1 
...
evenoddf = fix (\f arg -> (evenp arg, oddp arg))
...
denot (IfE be s1 s2) = \s -> if valueb be s then denot s1 s
                                else denot s2 s 
denot (WhileE be s1) = fix (\f s -> 
        if valueb be s then f (denot s1 s) 
        else s)
...
algs (SeqF s1 s2) = \s -> s2 (s1 s)
algs (IfEF be s1 s2) = \s -> if valueb be s then s1 s
                                else s2 s 
algs (WhileEF be s1) = fix (\f s -> 
        if valueb be s then f (s1 s) 
        else s)
