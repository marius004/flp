...
substaexp (Nu nr) str aexp = Nu nr
substaexp (Qid replace) str aexp = if replace == str then aexp else Qid replace
substaexp (PlusE expr1 expr2) str aexp = PlusE (substaexp expr1 str aexp) (substaexp expr2 str aexp)
substaexp (TimesE expr1 expr2) str aexp = TimesE (substaexp expr1 str aexp) (substaexp expr2 str aexp)
substaexp (DivE expr1 expr2) str aexp = DivE (substaexp expr1 str aexp) (substaexp expr2 str aexp)
...
valueassn (BEX v) state = v 
valueassn (LEX exp1 exp2) state = (value exp1 state) <= (value exp2 state) 
valueassn (NotEX exp1) state = not (valueassn exp1 state)
valueassn (AndEX exp1 exp2) state = (valueassn exp1 state) && (valueassn exp2 state)
valueassn (DisjInfX []) state = False
valueassn (DisjInfX (x:xs)) state = (valueassn x state) || (valueassn (DisjInfX xs) state)    
...
substassn (BEX value) str aexp = (BEX value)
substassn (LEX aexp1 aexp2) str aexp = (LEX (substaexp aexp1 str aexp) (substaexp aexp2 str aexp))
substassn (NotEX aexp1) str aexp = (NotEX (substassn aexp1 str aexp))
substassn (AndEX aexp1 aexp2) str aexp = (AndEX (substassn aexp1 str aexp) (substassn aexp2 str aexp))
substassn (DisjInfX xs) str aexp = DisjInfX (map (\x -> (substassn x str aexp)) xs)
...
wp :: Stmt -> Assn -> Assn
wp Skip assn = assn
wp (AtrE str aexp) assn = substassn assn str aexp
wp (Seq stmt1 stmt2) assn = wp stmt1 (wp stmt2 assn)
wp (IfE bexp stmt1 stmt2) assn = DisjInfX [AndEX (convassn bexp) (wp stmt1 assn), AndEX (NotEX (convassn bexp)) (wp stmt2 assn)]
wp (WhileE bexp stmt) assn = DisjInfX [eval i | i <- [0..]] 
    where eval 0 = AndEX (NotEX (convassn bexp)) assn 
          eval i = AndEX (convassn bexp) (wp stmt (eval (i - 1)))
